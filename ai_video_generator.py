...
GitHub Actions installs Colab CLI and calls:
  colab run --gpu T4 ai_video_generator.py

It receives scene prompts via a JSON file,
generates video clips using AI Video-T2V-1.3B,
...
Usage by pipeline.py:
  ...
  - colab run --gpu T4 ai_video_generator.py
  - colab download /content/clips/ ./ai_video_clips/
  - pipeline.py picks up clips from ./ai_video_clips/
"""
...
print(f"AI Video generator: {len(scenes)} scenes to generate")
...
for scene in scenes:
    ...
    try:
        ...
        if os.path.exists(out) and os.path.getsize(out) > 1000:
            ...
            results.append({"scene": n, "file": f"./ai_video_clips/{filename}", "success": True})
        else:
